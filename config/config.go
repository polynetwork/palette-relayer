...
func (c *ServiceConfig) PaletteKeystorePath() string {
	if path.IsAbs(c.PaletteConfig.KeyStorePath) {
		return c.PaletteConfig.KeyStorePath
	}
	return path.Join(c.Workspace, c.PaletteConfig.KeyStorePath)
}
...
func (c *ServiceConfig) ImportPaletteAccount(chainId *big.Int) (
	*keystore.PaletteKeyStore, []accounts.Account, error) {

	ks, err := keystore.NewPaletteKeyStore(c.PaletteKeystorePath(), chainId)
	if err != nil {
		return nil, nil, err
	}

	accArr := ks.GetAccounts()
	pwdSet := c.PaletteConfig.KeyStorePwdSet
	if len(pwdSet) == 0 {
...
func NewServiceConfig(configFilePath string) *ServiceConfig {
	...
	for k, v := range cfg.PaletteConfig.KeyStorePwdSet {
		delete(cfg.PaletteConfig.KeyStorePwdSet, k)
		cfg.PaletteConfig.KeyStorePwdSet[strings.ToLower(k)] = v
	}
